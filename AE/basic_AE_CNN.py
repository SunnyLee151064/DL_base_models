...
if __name__ == "__main__":
    ...
    for epoch in range(epoches):
        ...
        if (epoch+1) % 5 == 0:
            ...
            save_image(pic, './simple_autoencoder/image_{}_CNN.png'.format(epoch + 1))
    torch.save(model, './autoencoder_CNN.pth')
    model = torch.load('./autoencoder_CNN.pth')
    code = Variable(torch.randn(size=(1,32)).cuda())
    decode = model.decoder(code)
    decode_img = to_img(decode).squeeze()
    decode_img = decode_img.data.cpu()
    save_image(decode_img,"./simple_autoencoder/basic_reconstruction_CNN.png")
